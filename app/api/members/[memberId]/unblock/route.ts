...
// POST /api/members/[memberId]/unblock - Unblock a member
export async function POST(request: NextRequest, { params }: RouteParams) {
    try {
        ...
        // Unblock member by setting status to Active
        const result = await memberService.updateMember(memberId, {
            status: "Active",
        });

        if (!result.success) {
            return errorResponse(result.error || "Failed to unblock member", 400);
        }
        ...
        if (memberData.branchId) {
            try {
                ...
                console.log(`[Unblock] Found ${adminUsers.length} branch admins for branch ${memberData.branchId}`);
                ...
                for (const admin of adminUsers) {
                    const notification = await notificationRepository.createAsync({
                        ...
                        message: `${memberData.name} has been unblocked and can now access the gym.`,
                    ...
                    console.log(`[Unblock] Created notification ${notification.id} for admin ${admin.id}`);
                }
            } catch (notifError) {
                console.error("[Unblock] Failed to create notifications:", notifError);
                // Don't fail the request if notifications fail
            }
        }

        return successResponse(result.data, "Member unblocked successfully");

    } catch (error) {
        console.error("Unblock member error:", error);
        return errorResponse("Failed to unblock member", 500);
    }
}
